...
The problem is in the idf method. The last line

  Math.log(number_of_documents / number_of_documents_with_term)

which is the return value of the method, performs integer division, which loses
information. The change

  Math.log(number_of_documents.to_f / number_of_documents_with_term)

will do floating point division, avoiding the loss of the remainder.

Additionally, to avoid division by zero, the function should return early with
a value of zero if no documents contain the term argument.
...
# expected outputs:
puts tfidf("cat", document1, documents).round(1) == 1.2
puts tfidf("cat", document2, documents).round(1) == 1.6
puts tfidf("cat", document3, documents).round(1) == 0

puts tfidf("quantum", document1, documents).round(1) == 5.5
puts tfidf("quantum", document2, documents).round(1) == 0
puts tfidf("quantum", document3, documents).round(1) == 0

puts tfidf("mastery", document1, documents).round(1) == 0
puts tfidf("mastery", document2, documents).round(1) == 0
puts tfidf("mastery", document3, documents).round(1) == 3.3

puts tfidf("some", document1, documents).round(1) == 0
puts tfidf("some", document2, documents).round(1) == 0.4
puts tfidf("some", document3, documents).round(1) == 0.4
